...
router.get('/students', authenticateToken, async (req, res) => {
  try {
    const [students] = await pool.execute(`
      SELECT c.id, c.name, c.student_number, c.age, c.class_id, c.created_at,
...
router.post('/students', authenticateToken, authorizeRole(['teacher']), async (req, res) => {
  try {
    const { name, student_number, age, class_id } = req.body;

    if (!name || !student_number || !age || !class_id) {
      return res.status(400).json({ error: '学生姓名、学号、年龄和班级ID不能为空' });
    }
    ...
    const [result] = await pool.execute(
      'INSERT INTO children (name, student_number, age, class_id) VALUES (?, ?, ?, ?)',
      [name, student_number, age, class_id]
    );
...
router.put('/students/:id', authenticateToken, authorizeRole(['teacher']), async (req, res) => {
  try {
    ...
    // 更新学生信息
    await pool.execute(
      'UPDATE children SET name = ?, student_number = ?, age = ?, class_id = ? WHERE id = ?',
      [name, student_number, age, class_id, studentId]
    );
